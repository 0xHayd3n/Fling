...
export async function performLaunchSettings(
  params: PerformLaunchSettingsParams
...
export function registerLaunchSettings(server: McpServer): void {
  server.registerTool(
    "launch_settings",
    {
      ...
      inputSchema: {
        action: z
          ...
          .min(1)
          .describe(
            "Settings action. Bare suffix (WIFI_SETTINGS) or fully qualified (android.settings.WIFI_SETTINGS). Must be on the allowlist."
          ),
        data_uri: z
          .string()
          .min(1)
